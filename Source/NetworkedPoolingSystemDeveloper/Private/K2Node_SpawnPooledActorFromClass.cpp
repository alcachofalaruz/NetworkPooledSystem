...
UK2Node_SpawnPooledActorFromClass::UK2Node_SpawnPooledActorFromClass(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	NodeTooltip = LOCTEXT("NodeTooltip", "Attempts to spawn a pooled Actor with the specified transform");
}
...
void UK2Node_SpawnPooledActorFromClass::MaybeUpdateCollisionPin(TArray<UEdGraphPin*>& OldPins)
{
...
void UK2Node_SpawnPooledActorFromClass::ReallocatePinsDuringReconstruction(TArray<UEdGraphPin*>& OldPins) 
{
...
FText UK2Node_SpawnPooledActorFromClass::GetMenuCategory() const
{
	return FText::FromString(TEXT("Pooling"));
}

void UK2Node_SpawnPooledActorFromClass::GetPinHoverText(const UEdGraphPin& Pin, FString& HoverTextOut) const
{
...
FSlateIcon UK2Node_SpawnPooledActorFromClass::GetIconAndTint(FLinearColor& OutColor) const
{
...
FText UK2Node_SpawnPooledActorFromClass::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	FText NodeTitle = NSLOCTEXT("K2Node", "SpawnActor_BaseTitle", "Spawn Pooled Actor from Class");
...
bool UK2Node_SpawnPooledActorFromClass::IsCompatibleWithGraph(const UEdGraph* TargetGraph) const 
{
...
void UK2Node_SpawnPooledActorFromClass::PostPlacedNewNode()
{
...
void UK2Node_SpawnPooledActorFromClass::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
	Ar.UsingCustomVersion(FUE5MainStreamObjectVersion::GUID);
}

void UK2Node_SpawnPooledActorFromClass::PostLoad()
{
	FixupScaleMethodPin();
	Super::PostLoad();
}
...
void UK2Node_SpawnPooledActorFromClass::GetNodeAttributes( TArray<TKeyValuePair<FString, FString>>& OutNodeAttributes ) const
{
...
FNodeHandlingFunctor* UK2Node_SpawnPooledActorFromClass::CreateNodeHandler(FKismetCompilerContext& CompilerContext) const
{
	return new FNodeHandlingFunctor(CompilerContext);
}

void UK2Node_SpawnPooledActorFromClass::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	static const FName BeginSpawningBlueprintFuncName = GET_FUNCTION_NAME_CHECKED(UPoolSubsystem, K2_BeginSpawningPoolActor);
	static const FName ActorClassParamName(TEXT("ActorClass"));
	static const FName WorldContextParamName(TEXT("WorldContextObject"));

	static const FName FinishSpawningFuncName = GET_FUNCTION_NAME_CHECKED(UPoolSubsystem, K2_FinishSpawningPoolActor);
	...
	CallBeginSpawnNode->FunctionReference.SetExternalMember(BeginSpawningBlueprintFuncName, UPoolSubsystem::StaticClass());
	...
	CallFinishSpawnNode->FunctionReference.SetExternalMember(FinishSpawningFuncName, UPoolSubsystem::StaticClass());
	...
	UEdGraphPin* CallFinishContextPin = CallBeginSpawnNode->FindPinChecked(WorldContextParamName);
